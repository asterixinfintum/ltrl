...
connectbtnheader.addEventListener('click', function () {
    if (buttonlabel === 'Connect wallet') {
        connectWallet();
    } else {
        buyasset();
...
document.addEventListener('click', function (event) {
    ...
    if (
        inputmodal.style.display === 'flex' &&
        event.target.id !== 'toggleassetdropdown' &&
        event.target.id !== 'buttonname' &&
        event.target.id !== 'dropdownlogo' &&
        event.target.id !== 'buttonsvg'
    ) {
...
async function connectWallet() {
    try {
        ...
        document.getElementById('connectbtn').textContent = 'Buy $HPLT';

        const optiontext = document.getElementById('buttonname').textContent;
        currentasset = optiontext;

        contract = new ethers.Contract(
            "0x26d51f542eE2f33473d6e5fF4b8CFa54841eFFf7",
            contractabi,
            signer
        );

        assetContract = new ethers.Contract(assetAddress, assetAbi, signer);
...
async function approveAndCallReceiveUSDT() {
    document.getElementById('connectbtn').textContent = 'Processing...';
    const amountToSend = ethers.utils.parseUnits(value.toString(), 18); // Using 18 decimals for USDT
...
async function returnUserBalance(asset) {
    ...
    try {
        if (signeraddress) {
            ...
            let balanceFormatted = ethers.utils.formatUnits(userBalance, decimals);
